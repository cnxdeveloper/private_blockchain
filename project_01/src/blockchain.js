...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            let message2signed = `${address}:${new Date().getTime().toString().slice(0, -3)}:starRegistry`;
            resolve(message2signed)
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (!checkValid){
                reject("wallet address or signature invalid");
            }
            let newBlock = new BlockClass.Block({"address":address, "star": star, "sigmature":signature, "message": message});
    ...
    getBlockByHash(hash) {
        ...
        return new Promise(async (resolve, reject) => {
            try{
                let blocks = self.chain.filter(blk => blk.hash === hash)[0];
                console.log(blocks)
                if(blocks){
                    resolve(blocks);
                }else{
